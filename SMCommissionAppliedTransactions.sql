...
/****** Object:  StoredProcedure [dbo].[SMCommissionAppliedTransactions]    Script Date: 11/21/2022 1:40:33 PM ******/
SET ANSI_NULLS ON
GO
...
-- Description:	Fetch commission SM1, Rewrite SQL Code using Internal reconciliation function allows the report to match transactions posted to business partner accounts.
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[SMCommissionAppliedTransactions] 
...
BEGIN
	...
	(
		...
		SELECT 
			...
			T2.VatSum,
			T5.U_SM2,
			(SELECT TB.SlpName FROM OSLP TB WHERE TB.SlpCode=T5.U_SM2)				AS 'SecondSlp',
			IR.BalDueDeb
